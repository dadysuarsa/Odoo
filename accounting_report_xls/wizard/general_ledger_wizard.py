    ...
    def view_gl_report(self):
        ...
        query = """
                select 
                    ...
                    rp.name as partner, aml.name as label, string_agg(aa1.code, ', ') as counterpart, string_agg(aa1.name, ', ') as counterpart_name, aml.debit, aml.credit
                ...
                where 
                    ...
                    am.state in %s %s
                group by
                    aml.date, aml.divisi, mb.name, statement.name, statement.mutasi_bank_id, aml.id, am.name, aj.code, aa.code, aa.name , rp.name, aml.name, aml.debit, aml.credit
