...
class piutang_detail_idr_xls(report_xls):
    ...
    def generate_xls_report(self, parser, _xs, data, obj, wb):
        ...
        ws.write_merge(row_count-2, row_count-2, 6, 16, 'PELUNASAN', cell_style_center)
        ws.write_merge(row_count-1, row_count-1, 6, 8, 'INFO PEMBAYARAN', cell_style_center)
        ws.write_merge(row_count-1, row_count-1, 9, 9, 'PEMBAYARAN BANK/CASH', cell_style_center)
        ws.write_merge(row_count-1, row_count-1, 10, 10, 'RETUR', cell_style_center)
        ws.write_merge(row_count-1, row_count-1, 11, 11, 'KLAIM', cell_style_center)
        ws.write_merge(row_count-1, row_count-1, 12, 12, 'BANK CHARGES', cell_style_center)
        ws.write_merge(row_count-1, row_count-1, 13, 13, 'KOMISI', cell_style_center)
        ws.write_merge(row_count-1, row_count-1, 14, 14, 'POTONGAN', cell_style_center)
        ws.write_merge(row_count-1, row_count-1, 15, 15, 'LAIN-LAIN', cell_style_center)
        ws.write_merge(row_count-1, row_count-1, 16, 16, 'PEMBULATAN', cell_style_center)
        ...
        for partner in sorted(data['csv'].items(), key=operator.itemgetter(0)):
            ...
            row_start = row_count
            for bill in partner[1]['invoices']:
                col_count = 0
                for data_bill in partner[1]['invoices'][bill]['invoice_info']:
                    row_count += 1
                    for line in data_bill:
                        ws.write(row_count, col_count, line, c_cell_style)
                        col_count += 1
                ws.write(row_count, 17, partner[1]['invoices'][bill]['saldo_ahir'], c_cell_style_bold)
                has_payment = False
                for payments in partner[1]['invoices'][bill]['payments']:
...
piutang_detail_idr_xls('report.piutang.detail.idr.xls', 'account.invoice', 'addons/accounting_report_xls/report/report_excel.mako', parser=ReportStatus, header=False)