...
class bill_summary_xls(report_xls):
 
    def generate_xls_report(self, parser, _xs, data, obj, wb):
        ws = wb.add_sheet(('Laporan Bills (rekap)'))
        ...
        ws.write_merge(0, 0, 0, 14, 'VENDOR BILLS (REKAP) - ' + data['company_name'], title_style)
            ...
            for bill in partner[1]['bills']:
                ...
                for lines in partner[1]['bills'][bill]['grir']:
                    ...
                    for line in lines[:10]:
                        ...
                        col_count += 1
                    ws.write(row_count, col_count, lines[-1] + lines[-2], c_cell_style)
                    row_count +=1
                ...
                ws.write_merge(row_count, row_count, 4, 13, 'TOTAL %s : %s' % (partner[1]['bills'][bill]['number'], partner[0]), cell_style_grey)
                sum_cell_start = xlwt.Utils.rowcol_to_cell(sum_row, 14)
                sum_cell_end = xlwt.Utils.rowcol_to_cell(row_count - 1, 14)                    
                ws.write(row_count, 14, xlwt.Formula('sum(' + sum_cell_start + ':' + sum_cell_end + ')'), cell_style_grey)
        ...
        #GRAND TOTAL
        row_count += 1
        ws.write_merge(row_count, row_count, 4, 13, 'GRAND TOTAL', cell_style_grey)
        ws.write_merge(row_count, row_count, 14, 14, data['grand_total'][-1], cell_style_grey)
            
        pass
 
bill_summary_xls('report.bill.summary.xls', 'account.move.line', 'addons/accounting_report_xls/report/report_excel.mako', parser=ReportStatus, header=False)
