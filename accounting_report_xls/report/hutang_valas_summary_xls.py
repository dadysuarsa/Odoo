...
class hutang_valas_summary_xls(report_xls):
    ...
    def generate_xls_report(self, parser, _xs, data, obj, wb):
        ...
        ws.write_merge(row_count-2, row_count-2, 8, 12, 'PENGURANGAN', cell_style_center)
        ...
        for partner in sorted(data['csv'].items(), key=operator.itemgetter(0)):
            ...
            for bill in partner[1]['bills']:
                ...
                ws.write(row_count, 11, partner[1]['bills'][bill]['total_pelunasan_valas'], c_cell_style_bold)
                ws.write(row_count, 12, partner[1]['bills'][bill]['total_pelunasan_idr'], c_cell_style_bold)
                
                ws.write(row_count, 13, partner[1]['bills'][bill]['total_selisih_idr'], c_cell_style_bold)
                
                ws.write(row_count, 14, partner[1]['bills'][bill]['saldo_ahir_valas'], c_cell_style_bold)
                ws.write(row_count, 15, partner[1]['bills'][bill]['saldo_ahir_idr'], c_cell_style_bold)
