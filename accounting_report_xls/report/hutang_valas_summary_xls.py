...
class hutang_valas_summary_xls(report_xls):
 
    def generate_xls_report(self, parser, _xs, data, obj, wb):
        ws = wb.add_sheet(('Laporan Hutang Rekap (Valas)'))
        ...
        ws.write_merge(0, 0, 0, 14, 'LAPORAN HUTANG REKAP (Valas) - ' + data['company_name'], title_style)
        ...
        ws.write_merge(row_count-1, row_count-1, 6, 7, 'PENAMBAHAN', cell_style_center)

        ws.write_merge(row_count-2, row_count-2, 8, 14, 'PENGURANGAN', cell_style_center)
        ...
        ws.write_merge(row_count-1, row_count-1, 16, 17, 'SALDO AHIR', cell_style_center)
        ...
        for partner in sorted(data['csv'].items(), key=operator.itemgetter(0)):
            ...
            for bill in partner[1]['bills']:
                ...
                for data_bill in partner[1]['bills'][bill]['bill_info']:
                    ...
                    col_count += 1
                    ws.write(row_count, col_count, data_bill[8], c_cell_style)
                    col_count += 1
                    ws.write(row_count, col_count, data_bill[9], c_cell_style)
                    col_count += 1
                    ws.write(row_count, col_count, data_bill[10], c_cell_style)
                    col_count += 1
                    
                ws.write(row_count, 13, partner[1]['bills'][bill]['total_pelunasan_valas'], c_cell_style_bold)
                ws.write(row_count, 14, partner[1]['bills'][bill]['total_pelunasan_idr'], c_cell_style_bold)
                
                ws.write(row_count, 15, partner[1]['bills'][bill]['total_selisih_idr'], c_cell_style_bold)
                
                ws.write(row_count, 16, partner[1]['bills'][bill]['saldo_ahir_valas'], c_cell_style_bold)
                ws.write(row_count, 17, partner[1]['bills'][bill]['saldo_ahir_idr'], c_cell_style_bold)
...
hutang_valas_summary_xls('report.hutang.valas.summary.xls', 'account.invoice', 'addons/accounting_report_xls/report/report_excel.mako', parser=ReportStatus, header=False)