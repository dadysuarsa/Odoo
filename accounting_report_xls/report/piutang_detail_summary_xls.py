...
class piutang_summary_xls(report_xls):
    ...
    def generate_xls_report(self, parser, _xs, data, obj, wb):
        ...
        ws.write_merge(0, 0, 0, 14, 'LAPORAN PIUTANG (REKAP) - ' + data['company_name'], title_style)
        ...
        ws.write_merge(row_count-1, row_count-1, 4, 4, 'PENJUALAN', cell_style_center)

        ws.write_merge(row_count-2, row_count-2, 5, 8, 'PELUNASAN', cell_style_center)
        ...
        for partner in sorted(data['csv'].items(), key=operator.itemgetter(0)):
            ...
            for invoice in partner[1]['invoices']:
                ...
                for data_invoice in partner[1]['invoices'][invoice]['invoice_info']:
                    ...
                    col_count += 1
                ws.write(row_count, 9, partner[1]['invoices'][invoice]['saldo_ahir'], c_cell_style_bold)
                has_payment = False
        ...
        ws.write(row_count, col_count, data['grand_total'][5] + data['grand_total'][6] + data['grand_total'][7] + data['grand_total'][8] +\
                  data['grand_total'][9] + data['grand_total'][10] + data['grand_total'][11] + data['grand_total'][12], c_hdr_cell_style_grey)
        col_count += 1 
        ws.write(row_count, col_count, data['grand_total'][-1], c_hdr_cell_style_grey)
        col_count += 1 
...
piutang_summary_xls('report.piutang.summary.idr.xls', 'account.invoice', 'addons/accounting_report_xls/report/report_excel.mako', parser=ReportStatus, header=False)